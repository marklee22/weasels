class Pick < ActiveRecord::Base
  ...
  def validate_wildcards
    ...
    self.wildcard + Pick.where("user_id = ?", self.user_id).sum(:wildcard) <= MAX_WILDCARDS
    # error = "Your pick exceeds the maximum wildcards allowed: #{MAX_WILDCARDS}")
  end
  ...
  def validate_pick
    ...
    pick_found = false
    picks.each do |pick|
      logger.debug(pick.spread)
      logger.debug(pick.spread.year)
